package com.javarush.task16.task1613;
...
public class Solution {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        int returnedDay = dateTime.getDayOfWeek().getValue();
        int returnedHour = dateTime.getHour();
        if (returnedDay == FRIDAY && returnedHour >= WEEKEND_START_FRIDAY_CUT_OFF_HOUR)
            return true;
        else if (returnedDay == SATURDAY)
            return true;
        else if (returnedDay == SUNDAY && returnedHour < WEEKEND_END_SUNDAY_CUT_OFF_HOUR)
            return true;
        else
            return false;
    }
